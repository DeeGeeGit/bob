...
### Step 8: Verify CLAUDE.md Compliance

**Check if any changed directories contain a CLAUDE.md with numbered invariants.**

```bash
# Find CLAUDE.md files in the repo
find . -mindepth 2 -name "CLAUDE.md" | head -20
```

**If CLAUDE.md files exist in changed directories:**

Read each CLAUDE.md and check whether the code changes contradict or affect any of its numbered invariants, axioms, or assumptions.

```bash
# Find changed .go files to determine which directories to check
git diff --name-only HEAD
```

For each changed directory that has a CLAUDE.md:
1. Read the CLAUDE.md.
2. Review the changed code in that directory.
3. Identify any numbered invariant that the new code violates or renders inaccurate.
4. Report the specific invariant number and the nature of the drift.

These are informational findings — the tester reports them, and the orchestrator decides routing.

**Report findings in the CLAUDE.md Compliance section of test results.**
...
[If CLAUDE.md files were found in changed directories:]

**Module: `path/to/module/`**
- Invariants potentially affected by code changes: [list invariant numbers and descriptions, or "none"]

[If no CLAUDE.md files in changed directories: "N/A — no documented modules in changed directories"]
...
- 100% coverage does not equal good tests
